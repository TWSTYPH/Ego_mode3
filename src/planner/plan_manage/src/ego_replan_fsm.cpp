...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    else if (target_type_ == TARGET_TYPE::PRESET_TARGET)
    {
      ...
      waypoint_sub_ = nh.subscribe("/target_reference_node/target_odom", 1, &EGOReplanFSM::reference_pathCallback, this);   
    }
    else
  ...
  void EGOReplanFSM::reference_pathCallback(const geometry_msgs::PoseStampedPtr &msg)
  {
    ...
    cout << msg->header.seq<<"   referenceg_pathCallbacgggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggk!" << endl;
    Eigen::Vector3d end_wp(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
    std::cout << "ref:" << wps_.size() <<"end_wp:"<< end_wp<<std::endl;
    for (int idx = 0; idx < wps_.size(); idx++) {
        std::cout << "idx" << idx << ":" << wps_[idx] << std::endl;
    }
